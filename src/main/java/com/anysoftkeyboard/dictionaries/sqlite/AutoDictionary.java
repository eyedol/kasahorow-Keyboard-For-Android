...
public class AutoDictionary extends SQLiteUserDictionaryBase {
    ...
    /**
     * Adds the word to the auto-dictionary, if it was used enough times, it will be promoted to the
     * user's dictionary
     *
    ...
    public boolean addWord(WordComposer word, AdditionType type, AnySoftKeyboard callingIme) {
        synchronized (mResourceMonitor) {
            ...
            final int frequencyDelta = type.equals(AdditionType.Picked) ? FREQUENCY_FOR_PICKED
                    : FREQUENCY_FOR_TYPED;
    ...
    public boolean addWord(String word, int frequency) {
        throw new RuntimeException(
                "You should not call addWord(String, int) in AutoDictionary! Please call addWord(WordComposer, AdditionType)");
    }
}
