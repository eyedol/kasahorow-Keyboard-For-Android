...
public class ExternalDictionaryFactory extends AddOnsFactory<DictionaryAddOnAndBuilder> {
    ...
    public static ArrayList<DictionaryAddOnAndBuilder> getAllAvailableExternalDictionaries(
            Context askContext) {
        return msInstance.getAllAddOns(askContext);
    }

    public static DictionaryAddOnAndBuilder getDictionaryBuilderById(String id,
            Context askContext) {
        return msInstance.getAddOnById(id, askContext);
    }

    public static DictionaryAddOnAndBuilder getDictionaryBuilderByLocale(String locale,
            Context askContext) {
        return msInstance.getAddOnByLocale(locale, askContext);
    }

    private final HashMap<String, DictionaryAddOnAndBuilder> mBuildersByLocale
            = new HashMap<String, DictionaryAddOnAndBuilder>();

    private ExternalDictionaryFactory() {
        super(TAG, "com.menny.android.anysoftkeyboard.DICTIONARY",
    ...
    public synchronized DictionaryAddOnAndBuilder getAddOnByLocale(String locale,
            Context askContext) {
    ...
    @Override
    protected DictionaryAddOnAndBuilder createConcreteAddOn(Context askContext, Context context,
            String prefId, int nameId, String description, int sortIndex, AttributeSet attrs) {
        ...
        final int dictionaryResourceId = attrs
                .getAttributeResourceValue(null, XML_RESOURCE_ATTRIBUTE, AddOn.INVALID_RES_ID);
        final int autoTextResId = attrs
                .getAttributeResourceValue(null, XML_AUTO_TEXT_RESOURCE_ATTRIBUTE,
                        AddOn.INVALID_RES_ID);
        final int initialSuggestionsId = attrs
                .getAttributeResourceValue(null, XML_INITIAL_SUGGESTIONS_ARRAY_RESOURCE_ATTRIBUTE,
                        AddOn.INVALID_RES_ID);
        //asserting
    ...
    private void setIsDictionaryBought(Context context, String language) {

        try {
            BuyDictionaryFragment.checkUserHasBoughtDictionary(context,
                    new BuyDictionaryFragment.BuyDictionaryStatusListener() {

                        @Override
                        public void userBoughtDictionaryState(State s) {
                            switch (s) {
                                case NOT_AVAILABLE:
                                case BOUGHT:
                                    mIsBought = true;
            ...
            Log.w(TAG, "Purchased dictionary status failed", e);
        }
    }
}
